...
from app.ingest.fetcher import (RawReport, Timeframe, _fetch_events, _fetch_player_details,
                                _fetch_table, _list_reports, _load_report_detail)
...
async def _populate_boss_tables(guarded, client: WCLClient, raw: RawReport, encounter_id: int) -> None:
    ...
    for f in bf:
        raw.deaths_by_fight[f["id"]] = await guarded(
            _fetch_table(client, raw.code, "Deaths", float(f["startTime"]), float(f["endTime"]), [f["id"]]))

    # Boss-specific avoidable-damage events: Glaive hits on Midnight Falls. Fetched
    # per fight (one ability) so each hit's timestamp can be trimmed to the pull's
    # live portion downstream. Only this encounter, and only if configured.
    if encounter_id == settings.midnight_falls_encounter_id and settings.glaive_ability_id:
        for f in bf:
            raw.damage_taken_by_fight[f["id"]] = await guarded(
                _fetch_events(client, raw.code, "DamageTaken",
